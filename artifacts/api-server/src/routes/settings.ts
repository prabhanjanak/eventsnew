...
router.get("/settings/submissions", requireAuth(), async (_req, res): Promise<void> => {
  ...
  res.json({
    ...
    googleCallbackUrl: settings.googleCallbackUrl || null,
    googleWalletIssuerId: settings.googleWalletIssuerId || null,
    googleWalletServiceAccountEmail: settings.googleWalletServiceAccountEmail || null,
    googleWalletPrivateKey: settings.googleWalletPrivateKey || null,
...
router.patch("/settings/submissions", requireAuth(["super_admin"]), async (req, res): Promise<void> => {
  ...
  if (!settings) {
    [settings] = await db.insert(submissionSettingsTable).values({
      ...
      conferenceMapUrl: body.conferenceMapUrl || null,
      liveTvUrl: body.liveTvUrl || null,
      googleClientId: body.googleClientId?.trim() || null,
      googleClientSecret: body.googleClientSecret?.trim() || null,
      googleCallbackUrl: body.googleCallbackUrl?.trim() || null,
      googleWalletIssuerId: body.googleWalletIssuerId?.trim() || null,
      googleWalletServiceAccountEmail: body.googleWalletServiceAccountEmail?.trim() || null,
      googleWalletPrivateKey: body.googleWalletPrivateKey?.trim() || null,
      geminiApiKey: body.geminiApiKey?.trim() || null,
      hfToken: body.hfToken?.trim() || null,
      supportTicketCcEmails: body.supportTicketCcEmails || "saurabhrai@sankaraeye.com, prabhanjan@sankaraeye.com",
    }).returning();
  } else {
    [settings] = await db
      ...
      .set({
        ...
        googleSheetUrl: parsed.data.googleSheetUrl || null,
        conferenceMapUrl: body.conferenceMapUrl !== undefined ? (body.conferenceMapUrl || null) : settings.conferenceMapUrl,
        liveTvUrl: body.liveTvUrl !== undefined ? (body.liveTvUrl || null) : settings.liveTvUrl,
        googleClientId: body.googleClientId !== undefined ? (body.googleClientId?.trim() || null) : settings.googleClientId,
        googleClientSecret: body.googleClientSecret !== undefined ? (body.googleClientSecret?.trim() || null) : settings.googleClientSecret,
        googleCallbackUrl: body.googleCallbackUrl !== undefined ? (body.googleCallbackUrl?.trim() || null) : settings.googleCallbackUrl,
        googleWalletIssuerId: body.googleWalletIssuerId !== undefined ? (body.googleWalletIssuerId?.trim() || null) : settings.googleWalletIssuerId,
        googleWalletServiceAccountEmail: body.googleWalletServiceAccountEmail !== undefined ? (body.googleWalletServiceAccountEmail?.trim() || null) : settings.googleWalletServiceAccountEmail,
        googleWalletPrivateKey: body.googleWalletPrivateKey !== undefined ? (body.googleWalletPrivateKey?.trim() || null) : settings.googleWalletPrivateKey,
        geminiApiKey: body.geminiApiKey !== undefined ? (body.geminiApiKey?.trim() || null) : settings.geminiApiKey,
        hfToken: body.hfToken !== undefined ? (body.hfToken?.trim() || null) : settings.hfToken,
        ...(body.supportTicketCcEmails !== undefined ? { supportTicketCcEmails: body.supportTicketCcEmails?.trim() || null } : {}),
